// GAME LOGIC
export const GameLogic = {
    WIDTH: 1000,
    HEIGHT: 500,
    GRAVITY: 0.5,
    PLATFORM_WIDTH: 600,
    PLATFORM_HEIGHT: 10,
    PLATFORM_STARTING_X: 200,
    PLATFORM_STARTING_Y: 400,
    FRICTION: 0.5,
    LIFE_DELTA_MULTIPLIER: 1000,
    MANA_MULTIPLIER: 20,
    MAX_MANA: 100,
    HEALTH_MULTIPLIER: 50,
    TICK_DISCOUNT: 0.1,
    MS_PER_TICK: 16,
} as const;

// PLAYER LOGIC
export const PlayerLogic = {
    MAX_X_VEL_COMMANDED: 3,
    MAX_X_VEL: 8,
    MAX_Y_VEL: 10,
    JUMP_VEL_Y: -10,
    ATTACK_DAMAGE: 12,
    ALT_ATTACK_DAMAGE: 6,
    ALT_ATTACK_COST: 10,
    ALT_ATTACK_ORIGIN_Y_OFFSET: 10,
    MAX_HEALTH: 60,
    STARTING_HEALTH: 60,
    MAX_MANA: 100,
    STARTING_MANA: 20,
    TICKS_PER_MANA_CHARGE: 5,
    STARTING_LIVES: 3,
    PLAYER_WIDTH: 20,
    PLAYER_HEIGHT: 30,
    STARTING_POS_X: GameLogic.WIDTH/2,
    STARTING_POS_Y: GameLogic.HEIGHT/2,
    STARTING_VEL_X: 0,
    STARTING_VEL_Y: 0,
    KNOCKBACK_VEL_X: 5,
    KNOCKBACK_VEL_Y: -5,
    KNOCKBACK_VEL_X_BLOCK: 8,
    KNOCKBACK_TICKS: 20,
    INACTIVE_KILL_THRESHOLD: 200,
    MIN_VEL_X: 0.5,
    ARM_REACH: 20,
    ARM_SPEED: 5,
    ARM_HEIGHT: 5,
    ARM_Y_OFFSET: -10,
    PROJECTILE_VEL_X: 10,
    PROJECTILE_WIDTH: 10,
    PROJECTILE_HEIGHT: 2,
} as const;

// PLAYER RENDER
export const PlayerRender = {
    COLOR: [197, 254, 0],
    ALT_COLOR: [254, 176, 41],
    CHARGING_COLOR: [1, 254, 254],
    BLOCKING_COLOR: [116,48,218],
    EYE_COLOR: [0,0,0],
    EYE_OFFSET_X: 5,
    EYE_OFFSET_Y: 10,
    EYE_RADIUS_OUTER: 5,
    EYE_RADIUS_INNER: 2,
    ARM_COLOR: [0,0,0],
    PROJECTILE_COLOR: [0,0,0],
} as const;

//PLAYER INFO RENDER
export const PlayerInfoRender = {
    INFO_CONTAINER_COLOR: [0, 0, 0],
    INFO_CONTAINER_LEFT_POS_X: GameLogic.WIDTH/2 - 400,
    INFO_CONTAINER_RIGHT_POS_X: GameLogic.WIDTH/2 + 200,
    INFO_CONTAINER_POS_Y: GameLogic.HEIGHT/2 - 150,
    INFO_BORDER_X_OFFSET: 10,
    INFO_BORDER_Y_OFFSET: 15,
    INFO_BORDER_WIDTH: 250,
    INFO_BORDER_HEIGHT: 60,
    HEALTH_BAR_Y_OFFSET: 10,
    HEALTH_BAR_WIDTH: PlayerLogic.MAX_HEALTH + 2,
    HEALTH_BAR_HEIGHT: 10,
    HEALTH_BAR_COLOR: [200,100,100],
    HEALTH_BAR_INNER_X_OFFSET: 2,
    HEALTH_BAR_INNER_Y_OFFSET: 12,
    HEALTH_BAR_INNER_HEIGHT: 6,
    MANA_BAR_COLOR: [100,100,200],
    MANA_BAR_Y_OFFSET: 25,
    MANA_BAR_WIDTH: GameLogic.MAX_MANA + 2,
    MANA_BAR_HEIGHT: 10,
    MANA_BAR_INNER_X_OFFSET: 2,
    MANA_BAR_INNER_Y_OFFSET: 27,
    MANA_BAR_INNER_HEIGHT: 6,
    FITNESS_X_OFFSET: 70,
    LIVE_COUNT_X_OFFSET: 180,
    INFO_SIZE_SCALING: 2,
} as const;

// GAME RENDER
export const GameRender = {
    BACKGROUND_COLOR: [255, 255, 255],
    BACKGROUND_WIDTH: 1000,
    BACKGROUND_HEIGHT: 500,
    PLATFORM_COLOR: [27, 0, 46],
} as const;

